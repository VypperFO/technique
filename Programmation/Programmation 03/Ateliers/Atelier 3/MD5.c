/// \file MD5.c
/// \brief https://dev.w3.org/libwww/modules/md5/md5.h
/// \author RSA Data Security, Inc. Created 1991. All rights reserved.
...
void MD5Update(MD5_CTX* context, unsigned char* input, unsigned int length) {
  ...
  unsigned int i;
  if (length >= partLen) {
    for (unsigned int k = 0; k < partLen; k++)
      ((unsigned char*)&context->buffer[index])[k] = ((unsigned char*)input)[k];
  ...
  for (unsigned int k = 0; k < length - i; k++)
    ((unsigned char*)&context->buffer[index])[k] = ((unsigned char*)&input[i])[k];
}
...
void MD5Final(MD5_CTX* context, unsigned char* digest) {
  ...
  for (unsigned int i = 0; i < sizeof(*context); i++)
    ((unsigned char*)context)[i] = 0;
}
